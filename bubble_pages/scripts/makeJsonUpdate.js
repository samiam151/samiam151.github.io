...
d3.csv('data/DCPS_Master_114_sankey.csv', function(data){
    ...
    var maxExpend = d3.max(data, function(d){ return +d.MajorExp9815; }),
        ...
        maxFutureSpend = d3.max(data, function(d){ return +d.TotalAllotandPlan1621; }),
        minFutureSpend = d3.min(data, function(d){ return +d.TotalAllotandPlan1621; }),
        toScale = d3.scale.linear().domain([+minExpend, +maxExpend]).rangeRound([3, 35]),
        toUpdateScale = d3.scale.linear().domain([+minFutureSpend, +maxFutureSpend]).rangeRound([5, 35]);
    ...
    for(var i = 0, j = elementarySchools.length; i < j; i++){
        ...
        if(elementarySchools[i].MajorExp9815){            
            var value = getValue(elementarySchools);
            tempObj.value = value;
        }
        links.push(tempObj);
    }
    ...
    for(var i = 0, j = joinedSchools.length; i < j; i++){
        ...
        // SET VALUE           
        if(joinedSchools[i].TotalAllotandPlan1621){
    ...
    function getValue(arr, item){
        var token = arr ? arr[i] : item;
        if(token){
            var initialValue;
            // value = array's TotalAllot1621 scaled value
            if(token.TotalAllotandPlan1621){
                initialValue = token.TotalAllotandPlan1621;

                // numbers greater than 0, scale,
                // all zeroes return 10,
                // numbers less than 0, scale the abs value 
                
                // if(initialValue > 0){
                //     return toUpdateScale(initialValue);
                ...
                console.log('arr[i].TotalAllotandPlan1621 does not exist');
                return; 
            }
        }
    }
